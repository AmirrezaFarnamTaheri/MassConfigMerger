...
def create_app(settings: Settings) -> Flask:
    ...
    def api_export(format: str):
        ...
        if format == "csv":
            ...
            return send_file(BytesIO(csv_data.encode()), mimetype="text/csv", as_attachment=True, download_name=f"vpn_nodes_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
        elif format == "json":
            json_data = dashboard_data.export_json(nodes)
            return send_file(BytesIO(json_data.encode()), mimetype="application/json", as_attachment=True, download_name=f"vpn_nodes_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
