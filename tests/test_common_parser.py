...
from massconfigmerger.core.parsers.common import sanitize_headers, sanitize_str


def test_sanitize_str_non_string():
    """Test that sanitize_str returns non-string values as is."""
    assert sanitize_str(123) == 123
    assert sanitize_str(None) is None
    assert sanitize_str(["a", "b"]) == ["a", "b"]
...
def test_sanitize_headers_invalid_string():
    ...
    assert sanitize_headers(invalid_string) == invalid_string
...
def test_sanitize_headers_plain_json_string():
    ...
    assert sanitize_headers(json_string) == headers_dict
...
def test_sanitize_headers_base64_encoded_json():
    ...
    assert sanitize_headers(b64_string) == headers_dict
...
def test_sanitize_headers_dict():
    ...
    assert sanitize_headers(headers_dict) == expected_dict
...
def test_sanitize_headers_non_dict_after_decode():
    ...
    b64_string = base64.urlsafe_b64encode(json_string.encode()).decode()
    assert sanitize_headers(b64_string) == "not a dict"