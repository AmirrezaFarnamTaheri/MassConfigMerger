...
def test_write_all_outputs_html_enabled(mock_settings_fs: Settings, mock_results: list[ConfigResult], fs):
...
def test_write_all_outputs_surge_enabled(mock_settings_fs: Settings, mock_results: list[ConfigResult], fs):
    ...
    with patch("massconfigmerger.output_writer.generate_surge_conf", return_value=surge_content), \
        ...
        assert str(surge_file) in [str(p) for p in written_files]
        assert surge_file.read_text() == surge_content

def test_write_all_outputs_qx_enabled(mock_settings_fs: Settings, mock_results: list[ConfigResult], fs):
...
def test_write_all_outputs_advanced_formats_no_proxies(mock_settings_fs: Settings, fs):
