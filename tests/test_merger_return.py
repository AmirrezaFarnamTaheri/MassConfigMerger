...
async def test_run_merger_returns_list():
    ...
    # Run the merger
    results = await run_merger(settings, Path(settings.sources.sources_file))
...
async def test_run_merger_result_structure():
    ...
    if len(results) > 0:
        ...
        assert hasattr(first_result, 'config')
        assert hasattr(first_result, 'protocol')
        assert hasattr(first_result, 'ping_time')
        assert hasattr(first_result, 'host')
        assert hasattr(first_result, 'port')
        print(f"✓ Results have correct structure")
        print(f"  Sample: {first_result.protocol} - {first_result.host}:{first_result.port}")
