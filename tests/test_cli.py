...
from configstream.cli import download_geoip_db, main
...
def test_download_geoip_db_exists(fs):
    """
    Tests that the download function does nothing if the DB already exists.
    """
    # Create a fake existing database file
...
def test_download_geoip_db_success(fs):
    """
    Tests the successful download and extraction of the GeoIP database.
    """
...
def test_cli_merge_command(fs):
    ...
    runner = CliRunner()
    # We patch the pipeline and download_geoip_db to avoid external dependencies
    with patch("configstream.cli.pipeline.run_full_pipeline") as mock_pipeline, \
         patch("configstream.cli.download_geoip_db") as mock_download:
