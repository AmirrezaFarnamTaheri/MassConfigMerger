...
@pytest.mark.asyncio
@patch("massconfigmerger.tester.aiodns", new=None)
@patch("massconfigmerger.tester.AsyncResolver", new=None)
@patch("asyncio.open_connection")
async def test_node_tester_test_connection_success(mock_open_connection: MagicMock):
    """Test a successful connection in NodeTester without aiodns."""
    settings = Settings()
...
async def test_node_tester_with_dns_resolve(
    ...
    assert mock_resolver.resolve.await_args.args == ("example.com", 443)
...
async def test_lookup_country_success(
    mock_get_loop: MagicMock, MockReader: MagicMock
):
    ...
    mock_loop.getaddrinfo = AsyncMock(return_value=[(None, None, None, None, ("1.2.3.4", 0))])
