...
async def test_daemon_start(mock_settings, tmp_path):
    ...
    with patch("configstream.main_daemon.TestScheduler") as MockScheduler, \
        ...
        mock_run_dashboard.side_effect = lambda host, port: None

        await daemon.start(interval_hours=1, web_host="127.0.0.1", web_port=9000)

        MockScheduler.assert_called_once_with(mock_settings, tmp_path)
        mock_scheduler_instance.start.assert_called_once_with(1)
        mock_run_dashboard.assert_called_once_with(host="127.0.0.1", port=9000)
...
@patch("configstream.main_daemon.asyncio.run")
def test_main_function(mock_asyncio_run, mock_path_cls, mock_settings_cls, mock_daemon_cls):
    ...
    mock_daemon_instance.start.assert_called_once_with(
        interval_hours=2, web_host="0.0.0.0", web_port=8080
    )
